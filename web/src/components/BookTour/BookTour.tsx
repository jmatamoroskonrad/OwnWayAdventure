        ...
        {hasSlots ? (
          <>
            ...
            <div className="flex gap-2.5 flex-wrap">
              {slots.map((slot) => (
                <button key={slot.time} onClick={() => selectSlot(slot)} className={`px-5.5 py-4.25 rounded-[18px] border font-bricolage text-[18px] font-extrabold cursor-pointer transition duration-300 text-left min-h-16 hover:scale-105 ${slot.time === selectedSlot ? "scale-105 border-transparent bg-primary-background text-primary-foreground" : "border-primary-background/55 bg-transparent text-primary-text"}`}>
                  {slot.time}
          ...
          <span className="text-sm font-semibold text-primary-background/55">
            No time selection needed for this tour.
          </span>
        )}
        <div className="flex flex-wrap items-end gap-4.5">
          <div className="flex flex-col gap-3">
            ...
            <div className="flex items-center gap-1.5 p-1.5 rounded-full bg-primary-background/10 border border-primary-background/18">
              <button onClick={decreaseGuests} disabled={guests <= 1} className="w-13 h-13 border-0 rounded-full bg-primary-background text-primary-foreground text-[22px] font-bold cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed">
                −
              </button>
              <span
                data-dc-tpl="307"
              ...
              <button onClick={increaseGuests} disabled={atMaxGuests} className="w-13 h-13 border-0 rounded-full bg-primary-background text-primary-foreground text-[22px] font-bold cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed">
                +
              </button>
            </div>
          </div>
